...
#include "UnityCG.cginc"
	...
	half4 frag(v2f i) : SV_Target
	{
		...
		float restX = i.worldPos.x - floor(i.worldPos.x);
		float restZ = i.worldPos.z - floor(i.worldPos.z);
		float colorValue = 0.3f;
		return restX < 0.025 || restZ <0.025 ? half4((colorValue + bgcolor.r)/2, (colorValue + bgcolor.g) / 2, (colorValue + bgcolor.b) / 2,0) : bgcolor;
